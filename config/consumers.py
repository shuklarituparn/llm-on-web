...
async def chat(self, usermessage: str, pk):
    ...
    responsellm = ""
    async for part in await AsyncClient(host='http://212.109.220.83:11434').chat(
        model="llama3",
        messages=[message],
        stream=True,
    ):
...
async def chatllava(self, usermessage: str, image: str, pk):
    conv_id = str(uuid.uuid4())
    responsellm_llava = ""
    async for response in await AsyncClient(host='http://212.109.220.83:11434').generate(
        "bakllava",
        usermessage,
        images=[image],
        stream=True,
